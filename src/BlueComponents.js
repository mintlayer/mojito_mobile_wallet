...
import { create_wallet, add_wallet, bitcoin_small, circle_logo } from './theme/Images';
...
export const MintLayerButton = (props) => {
  const { colors } = useTheme();
  return (
    <TouchableOpacity accessibilityRole="button" testID={props.testID} onPress={props.onPress}>
      <View
      ...
      >
        <View style={{ marginHorizontal: 16, marginVertical: 10, flexDirection: 'row', alignItems: 'center' }}>
          <View>
            <Image style={{ width: 34, height: 34, marginRight: 8 }} source={circle_logo} />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={{ color: colors.walletBalanceBgColor, fontWeight: 'bold', fontSize: 18, writingDirection: I18nManager.isRTL ? 'rtl' : 'ltr' }}>{props.title}</Text>
            <Text
              style={{
                color: colors.alternativeTextColor,
                fontSize: 13,
