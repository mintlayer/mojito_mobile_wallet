...
import { HDSegwitElectrumSeedP2WPKHWallet, HDLegacyBreadwalletWallet, HDSegwitBech32Wallet, HDLegacyElectrumSeedP2PKHWallet, LegacyWallet, SegwitP2SHWallet, SegwitBech32Wallet, HDLegacyP2PKHWallet, HDSegwitP2SHWallet, WatchOnlyWallet, HDAezeedWallet, SLIP39SegwitP2SHWallet, SLIP39SegwitBech32Wallet } from '../../class';
...
describe('import procedure', () => {
  ...
  it('can import BIP84', async () => {
    const store = createStore();
    const { promise } = startImport('always direct find escape liar turn differ shy tool gap elder galaxy lawn wild movie fog moon spread casual inner box diagram outdoor tell', false, false, ...store.callbacks);
    await promise;
  ...
  it('can import BIP84 with passphrase', async () => {
    ...
    const { promise } = startImport('abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about', true, false, ...store.callbacks);
    await promise;
    assert.strictEqual(store.state.wallets[0].type, HDSegwitBech32Wallet.type);
  ...
  it('can import HD Legacy Electrum (BIP32 P2PKH)', async () => {
    const store = createStore();
    const { promise } = startImport('eight derive blast guide smoke piece coral burden lottery flower tomato flame', false, false, ...store.callbacks);
    await promise;
  ...
  it('can import HD Legacy Electrum (BIP32 P2PKH) with passphrase', async () => {
    ...
    const { promise } = startImport('receive happy wash prosper update pet neck acid try profit proud hungry', true, false, ...store.callbacks);
    await promise;
    assert.strictEqual(store.state.wallets[0].type, HDLegacyElectrumSeedP2PKHWallet.type);
  ...
  it('can import BreadWallet', async () => {
    const store = createStore();
    const { promise } = startImport('become salmon motor battle sweet merit romance ecology age squirrel oblige awesome', false, false, ...store.callbacks);
    await promise;
  ...
  it('can import HD Electrum (BIP32 P2WPKH)', async () => {
    const store = createStore();
    const { promise } = startImport('noble mimic pipe merry knife screen enter dune crop bonus slice card', false, false, ...store.callbacks);
    await promise;
  ...
  it('can import HD Electrum (BIP32 P2WPKH) with passphrase', async () => {
    ...
    const { promise } = startImport('bitter grass shiver impose acquire brush forget axis eager alone wine silver', true, false, ...store.callbacks);
    await promise;
    assert.strictEqual(store.state.wallets[0].type, HDSegwitElectrumSeedP2WPKHWallet.type);
  ...
  it('can import AEZEED', async () => {
    const store = createStore();
    const { promise } = startImport('abstract rhythm weird food attract treat mosquito sight royal actor surround ride strike remove guilt catch filter summer mushroom protect poverty cruel chaos pattern', false, false, ...store.callbacks);
    await promise;
  ...
  it('can import AEZEED with password', async () => {
    ...
    const { promise } = startImport('able mix price funny host express lawsuit congress antique float pig exchange vapor drip wide cup style apple tumble verb fix blush tongue market', false, false, ...store.callbacks);
    await promise;
    assert.strictEqual(store.state.wallets[0].type, HDAezeedWallet.type);
  });
  ...
  it('can import watch-only address', async () => {
    ...
    const { promise: promise4 } = startImport('zpub6r7jhKKm7BAVx3b3nSnuadY1WnshZYkhK8gKFoRLwK9rF3Mzv28BrGcCGA3ugGtawi1WLb2vyjQAX9ZTDGU5gNk2bLdTc3iEXr6tzR1ipNP', false, false, ...store4.callbacks);
    await promise4;
  ...
  it('can import slip39 wallet', async () => {
    ...
    const { promise } = startImport('crystal lungs academic acid corner infant satisfy spider alcohol laser golden equation fiscal epidemic infant scholar space findings tadpole belong\n' + 'crystal lungs academic agency class payment actress avoid rebound ordinary exchange petition tendency mild mobile spine robin fancy shelter increase', false, false, ...store.callbacks);
    await promise;
  ...
  it('can import slip39 wallet with password', async () => {
    ...
    // crystal lungs academic agency class payment actress avoid rebound ordinary exchange petition tendency mild mobile spine robin fancy shelter increase
    // crystal lungs academic always earth satoshi elbow satoshi that pants formal leaf rival texture romantic filter expand regular soul desert
    const { promise } = startImport('crystal lungs academic acid corner infant satisfy spider alcohol laser golden equation fiscal epidemic infant scholar space findings tadpole belong\n' + 'crystal lungs academic agency class payment actress avoid rebound ordinary exchange petition tendency mild mobile spine robin fancy shelter increase', true, false, ...store.callbacks);
    await promise;
  ...
  it('can import watch-only Cobo vault export', async () => {
    const store = createStore();
    const { promise } = startImport('{"ExtPubKey":"zpub6riZchHnrWzhhZ3Z4dhCJmesGyafMmZBRC9txhnidR313XJbcv4KiDubderKHhL7rMsqacYd82FQ38e4whgs8Dg7CpsxX3dSGWayXsEerF4","MasterFingerprint":"7D2F0272","AccountKeyPath":"84\'\\/0\'\\/0\'","CoboVaultFirmwareVersion":"2.6.1(BTC-Only)"}', false, false, ...store.callbacks);
    await promise;
  ...
  it('can import watch-only Keystone vault export', async () => {
    const store = createStore();
    const { promise } = startImport('{"ExtPubKey":"zpub6qT7amLcp2exr4mU4AhXZMjD9CFkopECVhUxc9LHW8pNsJG2B9ogs5sFbGZpxEeT5TBjLmc7EFYgZA9EeWEM1xkJMFLefzZc8eigRFhKB8Q","MasterFingerprint":"01EBDA7D","AccountKeyPath":"m/84\'/0\'/0\'"}', false, false, ...store.callbacks);
    await promise;
    assert.strictEqual(store.state.wallets[0].type, WatchOnlyWallet.type);
    assert.strictEqual(store.state.wallets[0].getDerivationPath(), "m/84'/0'/0'");
  });
  ...
  it('can import BIP39 wallets with truncated words', async () => {
    // 12 words
    const store1 = createStore();
    const { promise: promise1 } = startImport('trip ener cloc puls hams ghos inha crow inju vibr seve chro', false, false, ...store1.callbacks);
    await promise1;
    assert.strictEqual(store1.state.wallets[0].getSecret(), 'trip energy clock pulse hamster ghost inhale crowd injury vibrant seven chronic');

    // 16 words
    const store2 = createStore();
    const { promise: promise2 } = startImport('docu gosp razo chao nort ches nomi fati swam firs deca boy icon virt gap prep seri anch', false, false, ...store2.callbacks);
    await promise2;
    assert.strictEqual(store2.state.wallets[0].getSecret(), 'document gospel razor chaos north chest nominee fatigue swamp first decade boy icon virtual gap prepare series anchor');

    // 24 words
    const store3 = createStore();
    const { promise: promise3 } = startImport('rece own flig sent tide hood sile bunk deri mana wink belt loud apol mons pill raw gate hurd matc nigh wish todd achi', false, false, ...store3.callbacks);
    await promise3;
    assert.strictEqual(store3.state.wallets[0].getSecret(), 'receive own flight sentence tide hood silent bunker derive manage wink belt loud apology monster pill raw gate hurdle match night wish toddler achieve');
  });
});
